...
void Host_Fly_f ()
{
	...
	if (ed_float(sv_player, movetype) != MOVETYPE_FLY)
	{
...
void Host_Name_f ()
{
	...
	ed_set_string(host_client->edict, netname, host_client->name);
...
// turn on color set 1
	...
	for (j = 0, client = svs.clients; j < svs.maxclients; j++, client++)
	{
		if (!client || !client->active || !client->spawned)
			continue;
		if (teamplay.value && teamonly && ed_float(client->edict, team) != ed_float(save->edict, team))
			continue;
		host_client = client;
...
void Host_Color_f()
{
	...
	ed_float(host_client->edict, team) = bottom + 1;
...
void Host_Pause_f ()
{
	...
	else
	{
		...
		if (sv.paused)
		{
			SV_BroadcastPrintf ("%s paused the game\n", ed_get_string(sv_player, netname));
		}
		else
		{
...
void Host_Kick_f ()
{
	...
	else if (sv_pr_float(deathmatch) && !host_client->privileged)
		return;
...
edict_t	*FindViewthing ()
{
	...
	{
		e = EDICT_NUM(i);
		if ( !strcmp (ed_get_string(e, classname), "viewthing") )
			return e;
	}
...
void Host_Viewmodel_f ()
{
	...
	ed_float(e, frame) = 0;
	cl.model_precache[(int)ed_float(e, modelindex)] = m;
}
...
void Host_Viewframe_f ()
{
	...
	if (!e)
		return;
	m = cl.model_precache[(int)ed_float(e, modelindex)];
...
void Host_Viewnext_f ()
{
	...
	ed_float(e, frame)++;
	if (ed_float(e, frame) >= m->numframes)
		ed_float(e, frame) = m->numframes - 1;

	PrintFrameName (m, ed_float(e, frame));		
}
