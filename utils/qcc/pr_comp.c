...
opcode_t pr_opcodes[] = {{"<DONE>", "DONE", -1, false, &def_entity, &def_field, &def_void},
...
def_t *PR_ParseImmediate (void)
{
	...
	for (cn = pr.def_head.next; cn; cn = cn->next)
	{
		...
		{
			if ((G_FLOAT (cn->ofs) == pr_immediate.vector[0]) && (G_FLOAT (cn->ofs + 1) == pr_immediate.vector[1]) &&
...
def_t *PR_GetDef (type_t *type, char *name, def_t *scope, bool allocate)
{
	...
	{
		*(int32_t *)&pr_globals[def->ofs] = pr.size_fields;
