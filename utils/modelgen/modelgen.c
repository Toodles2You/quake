...
void WriteModelFile (FILE *modelouthandle)
{
	...
	model.boundingradius = sqrt (dist[0] * dist[0] + dist[1] * dist[1] + dist[2] * dist[2]);
...
void SetSkinValues (void)
{
	...
	for (i = 0; i < model.numtris; i++)
	{
		...
		VectorSubtract (baseverts[triangles[i].vertindex[0]], baseverts[triangles[i].vertindex[1]], vtemp1);
		VectorSubtract (baseverts[triangles[i].vertindex[2]], baseverts[triangles[i].vertindex[1]], vtemp2);
...
void Cmd_Base (void)
{
	...
	for (i = 0; i < model.numtris; i++)
	{
		if (VectorCompare (ptri[i].verts[0], ptri[i].verts[1]) || VectorCompare (ptri[i].verts[1], ptri[i].verts[2]) ||
