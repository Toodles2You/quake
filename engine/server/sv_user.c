...
static void SV_PreSpawn_f (void)
{
	...
	if (!buf)
	{
		...
		if (sv_mapcheck.value && check != sv.worldmodel->checksum && check != sv.worldmodel->checksum2)
		{
...
static void SV_Spawn_f (void)
{
	...
	{
		ClientReliableWrite_Begin (host_client, svc_lightstyle, 3 + (sv.lightstyles[i] ? strlen (sv.lightstyles[i]) : 1));
...
static void AddLinksToPmove (areanode_t *node)
{
	...
	for (l = node->solid_edicts.next; l != &node->solid_edicts; l = next)
	{
		...
		if (ed_float (check, solid) == SOLID_BSP || ed_float (check, solid) == SOLID_BBOX || ed_float (check, solid) == SOLID_SLIDEBOX)
		{
			if (check == sv_player)
				continue;

			for (i = 0; i < 3; i++)
				if (ed_vector (check, absmin)[i] > pmove_maxs[i] || ed_vector (check, absmax)[i] < pmove_mins[i])
					break;
			if (i != 3)
				continue;
