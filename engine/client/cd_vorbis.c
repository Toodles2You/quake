...
void CDAudio_Play (byte track, bool looping)
{
	...
	Con_DPrintf ("CDAudio: %s %s\nCDAudio: %i channels, %i hz\n", looping ? "Looping" : "Playing", trackname, info.channels, info.sample_rate);
...
void CDAudio_Paint (void *out, int idx, int count)
{
	...
	int recv = stb_vorbis_get_samples_short_interleaved (vorbis, 2, &bgm_buffer[0], count * trackrate);
	...
	if (recv < count)
	{
		...
		{
			recv = stb_vorbis_get_samples_short_interleaved (vorbis, 2, &bgm_buffer[recv], (count - recv) * trackrate);
		}
		else
		{
			count = recv;
